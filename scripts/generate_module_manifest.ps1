...
$defaults = @{
    ...
    'Copyright'         = ('(c) ' + (Get-Date -UFormat %Y) + ' Pavel Stsefanovich. All rights reserved.');
    'ModuleVersion'     = '0.1.0';
...
if (! $skip_manifest_update) {
    ...
    if ($NEW_MANIFEST) {
        ...
        foreach ( $key in ($manifest.Keys | ? { $_ -like '*Uri' }) ) {
            if ( ! (verify-public-url $manifest.$key) ) {
                $urls_valid = $false
    ...
    # UPDATE EXISTING MANIFEST
        ...
        if ($patch) { $module_version[2] = bump_version $module_version[2] }
        elseif ($minor) {
            $module_version[1] = bump_version $module_version[1]
            $module_version[2] = "0"
        }
        elseif ($major) {
            $module_version[0] = bump_version $module_version[0]
            $module_version[1] = "0"
            $module_version[2] = "0"
        }
